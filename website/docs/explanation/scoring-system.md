...
sidebar_position: 3
description: "The privacy x utility x quality scoring model and the output gate."
---
...
**File:** `pipeline/scoring/gate.py`

When `MEDANON_SCORING_ENABLED=true`, the score gate runs automatically after every bulk export job completes and before the job is marked `done`. If the gate fails, the output file is **deleted** and the job transitions to `failed` with a plain-language explanation of what leaked and how to fix it.

Two independent checks run in sequence:

### Hard PII leak gate (always active, cannot be lowered)
...
### Composite score gate (configurable threshold)

Blocks if `avg_composite < MEDANON_SCORE_GATE_MIN_COMPOSITE` (default 80, Grade B). Grades C (65-79%) and D (50-64%) are blocked by default.

The gate also fires on:
- Privacy gate failures: `fail_count > 0` resources with risk score above threshold.
- High error rate: > 5% of resources produced processing errors.

### Feedback on failure

When the gate blocks, the job record carries a structured `report` field with:
- `critical_pii`: whether detected PII was the trigger (vs. coverage gap or low score)
- `leaked_fields`: list of `{path, resource_count}` for uncovered HIPAA-sensitive paths
- `issues` + `fixes`: ordered plain-language list of what to change in the config profile
- `grade`, `score`, `resources_total`

This data is surfaced in the UI without requiring the operator to parse log output.

## Configuration

| Variable | Default | Effect |
|---|---|---|
| `MEDANON_SCORING_ENABLED` | `false` | Auto-score and persist every de-identification run to `medanon.processing_runs`. When `true`, the output gate is also active by default. |
| `MEDANON_SCORE_GATE_ENABLED` | _(follows `MEDANON_SCORING_ENABLED`)_ | Explicitly override gate enable/disable independent of scoring persistence. |
| `MEDANON_SCORE_GATE_MIN_COMPOSITE` | `80` | Minimum composite score to pass. Grade B = 80, Grade C = 65, Grade D = 50. |
| `MEDANON_SCORE_GATE_REQUIRE_PRIVACY` | `true` | Also block when privacy gate fail count > 0. |
| `MEDANON_GATE_IDENTIFIER_MODE` | `block` | `block` = hard block on uncovered HIPAA fields; `warn` = advisory when no actual PII was detected. |
