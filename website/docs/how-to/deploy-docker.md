---
title: "Deploy (Docker & Kubernetes)"
sidebar_position: 12
description: "Deploy the stack with Docker Compose and Kubernetes/Helm."
---
...
Starts all always-on services in dependency order. **gPAS (WildFly) takes ~90 seconds on first boot**, it deploys the TTP-FHIR WAR file and initializes the PostgreSQL schema.
...
docker compose --profile trust up        # Trust Gate + fhir-validator (pre-privacy data quality barrier)
docker compose --profile auth up         # Keycloak + Keycloak DB (OIDC identity provider, realm: medanon)
docker compose --profile monitoring up   # Prometheus + Grafana + cAdvisor + Jaeger
docker compose --profile sqltest up      # PostgreSQL fixture for SQL/tabular source testing
```

Profiles can be combined:

```bash
docker compose --profile trust --profile monitoring up -d
```

### Trust Gate profile

The `trust` profile starts three services: `trust-gate` (port 8400), `trust-gate-ui` (port 8401), and `fhir-validator`. Enable it in `.env` and configure the mode:

```bash
TRUST_GATE_SERVICE_URL=http://trust-gate:8400
TRUST_GATE_MODE=warn   # advisory passport (default) | block | off
```

See [Trust Gate](./trust-gate.md) for the full integration guide.

### Auth profile

The `auth` profile starts Keycloak (`keycloak`, port 8180) and its PostgreSQL backend (`keycloak-db`). When enabled, configure OIDC in `.env`:
