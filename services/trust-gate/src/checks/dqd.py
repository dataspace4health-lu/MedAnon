...
def run_dqd_checks(
    omop: OmopData, value_checks: dict | None = None
) -> list[CheckResult]:
    ...
    for t in REQUIRED_TABLES:
        ...
        checks.append(
            _ck(
                f"dqd.table.{t}.present",
                "conformance",
                "relational",
                "integrity",
                STRUCTURAL,
                applicable=1,
                violations=0 if present else 1,
                critical=True,
                description=f"OMOP table '{t}' is present",
