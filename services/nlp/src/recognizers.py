...
def _build_custom_recognizers():
    ...
    ], context=["postcode", "PLZ", "code postal", "código postal", "CAP", "Postleitzahl", "postnummer", "Sweden"])

    # --- Strong SSN (override Presidio built-in low-confidence pattern) ---
    _add("US_SSN", "us_ssn_strong_recognizer", [
        # Standard SSN format — high confidence even without context label
        ("ssn_dashed", r"\b(?!000|666|9\d{2})\d{3}-(?!00)\d{2}-(?!0000)\d{4}\b", 0.85),
    ], context=["SSN", "social security", "social-security"])

    # --- SWIFT / BIC code ---
    _add("SWIFT_BIC", "swift_bic_recognizer", [
    ...
    _add("AGE", "age_marker_recognizer", [
    ...
    _add("RACE_ETHNICITY", "race_ethnicity_recognizer", [
        ("race_eth", (
    ...
    _add("INSURANCE_STATUS", "insurance_status_recognizer", [
        ("uninsured", (
    ...
    _add("ORGANIZATION", "healthcare_org_recognizer", [
        # "<Name> Hospital/Clinic/..." — trailing keyword
        ("facility_suffix", (
            r"\b(?!(?:" + _ORG_DENY + r")\b)(?-i:[A-ZÀ-Ü])[\wÀ-ÿ'\-]+(?:\s+(?-i:[A-ZÀ-Ü])[\wÀ-ÿ'\-]+){0,3}"
            r"\s+(?:Hospital|Clinic|Medical\s+Center|Health\s+Center|Health\s+System|"
            r"Hôpital|Clinique|Centre\s+Hospitalier|Centre\s+Médical|Polyclinique|"
            r"Krankenhaus|Klinik|Klinikum|Universitätsklinikum|"
            r"Ospedale|Policlinico|Clinica|"
            r"Hospital|Clínica|Centro\s+Médico|"
            r"Ziekenhuis|Kliniek)\b"
        ), 0.8),
    ...
    ], context=["hospital", "clinic", "clinique", "krankenhaus", "ospedale", "insurance", "payer"])
    ...
    _add("GEO_COORDINATES", "geo_coordinates_recognizer", [
    ...
    _add("PERSON", "synthea_person_recognizer", [
    ...
    _add("MAC_ADDRESS", "mac_address_recognizer", [
    ...
    _add("CRYPTO_WALLET", "crypto_wallet_recognizer", [
    ...
    _add("BEARER_TOKEN", "bearer_token_recognizer", [
        # JWT: 3 base64url segments separated by dots
        ("jwt", r"\beyJ[A-Za-z0-9_\-]{8,}\.[A-Za-z0-9_\-]{8,}\.[A-Za-z0-9_\-]{8,}\b", 0.95),
    ...
    _add("GENETIC_VARIANT", "genetic_variant_recognizer", [
