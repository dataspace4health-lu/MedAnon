...
def run_fetch(args):
    ...
    if args.discover:
        try:
            resource_types = get_capability_statement(
                server, token=token, timeout=timeout
            )
        except ValueError as exc:
            raise SystemExit(
                f"error: FHIR server error during /metadata: {exc}"
            ) from exc
    ...
    if args.resource_types:
        resource_types = [
            rt.strip() for rt in args.resource_types.split(",") if rt.strip()
        ]
    else:
        try:
            resource_types = get_capability_statement(
        ...
        print(
            f"Found {len(resource_types)} resource type(s): {', '.join(resource_types)}"
        )
