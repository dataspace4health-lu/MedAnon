...
class PrivacyRiskEvaluator:
    ...
    def evaluate(
        ...
        identifier_risk = self._identifier_detection(deidentified, manifest_entries, evidence)
    ...
    def evaluate_batch(
        ...
        evidence.append(Evidence(
            ...
            details={"worst_resource_risk": identifier_risk, "count": len(id_risks)},
    ...
    def _attacker_model(self, deidentified: dict, evidence: list[Evidence]) -> float:
        ...
        try:
            ...
            evidence.append(Evidence(
                check="attacker_model", value=0.0,
        ...
        suppressed = sum(
            1 for v in qi if not v or v in REDACTED_SENTINELS
        )
        total = len(qi)
    ...
    def _attacker_model_batch(
        self, patients: list[dict], evidence: list[Evidence],
    ) -> float:
        try:
            ...
            evidence.append(Evidence(
                check="attacker_model_batch", value=0.0,
                details={"reason": "medanon_core not available"},
            ))
            return 0.0

        patient_resources = [
            p for p in patients if p.get("resourceType") == "Patient"
        ]
    ...
    def _identifier_detection(
        ...
        if not manifest_entries and rtype in PHI_RESOURCE_TYPES:
            evidence.append(Evidence(
                ...
                value=1.0,
                details={"reason": "no_transformations_detected", "resource_type": rtype},
                severity="critical",
            ))
            return 1.0
        ...
        for s_path in sensitive:
            if not any(s_path == cp or cp.startswith(s_path + ".") or s_path.startswith(cp + ".") for cp in covered_paths):
    ...
    def _walk_texts(
        self, obj: Any, texts: list[str], min_len: int, depth: int,
    ) -> None:
        if depth > 15:
            return
