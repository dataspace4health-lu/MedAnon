    ...
    if phase == "fetching":
        ...
        for ti, rt in enumerate(resource_types):
            ...
            for resource, next_url in fetch_resource_type(
                ...
                start_url=start, yield_cursors=True,
            ):
    ...
    if phase == "processing":
        ...
        with open(output_path, open_mode, encoding="utf-8") as fh:
            while True:
                fresh = store.get(job.id)
                if fresh and fresh.status == JobStatus.CANCELLED:
                    _log.info("staged_process_cancelled job=%s at=%d", job.id, processed)
                    return
    ...
    if phase == "processing":
        ...
        with open(output_path, open_mode, encoding="utf-8") as fh:
            while True:
                fresh = store.get(job.id)
                if fresh and fresh.status == JobStatus.CANCELLED:
                    _log.info("staged_cohort_process_cancelled job=%s at=%d", job.id, processed)
                    return
    ...
    if phase == "processing":
        ...
        with open(output_path, open_mode, encoding="utf-8") as fh:
            while True:
                fresh = store.get(job.id)
                if fresh and fresh.status == JobStatus.CANCELLED:
                    _log.info("staged_patient_process_cancelled job=%s at=%d", job.id, processed)
                    return
