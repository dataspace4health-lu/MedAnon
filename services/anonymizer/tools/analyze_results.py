...
def _collect_request_urls(obj, out):
    if isinstance(obj, dict):
        if isinstance(obj.get("request"), dict) and isinstance(
            obj["request"].get("url"), str
        ):
...
def analyze_pair(
    ...
    ref_changes, request_url_changes, url_key_changes = _metric_reference_url_changes(
        original, processed
    )
...
def _markdown_report(results):
    ...
    for idx, r in enumerate(results, start=1):
        ...
        lines.append(
            f"- ids changed: {r['ids_changed']} / {r['comparable_resource_ids']}"
        )
...
def main():
    parser = argparse.ArgumentParser(
        description="Analyze before/after FHIR transformation outputs."
    )
    parser.add_argument("--input", required=True)
    parser.add_argument("--output", required=True)
    parser.add_argument(
        "--input-format", default="auto", choices=["auto", "json", "ndjson", "xml"]
    )
    parser.add_argument(
        "--output-format", default="auto", choices=["auto", "json", "ndjson", "xml"]
    )
