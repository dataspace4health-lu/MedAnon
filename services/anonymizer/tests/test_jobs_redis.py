...
class TestRedisJobStore(unittest.TestCase):
    ...
    def test_notify_and_wait_for_job(self):
        """wait_for_job returns (job_id, message_id) tuple via Redis Streams."""
        job = self.store.create("bulk-export", {})
        self.store.notify_new_job(job.id)
        result = self.store.wait_for_job(timeout=2)
        self.assertIsNotNone(result)
        received_id, message_id = result
        self.assertEqual(received_id, job.id)
        self.assertIsNotNone(message_id)
        # Ack so the message is removed from the PEL
        self.store.ack_job(message_id)

    def test_wait_for_job_timeout_returns_none(self):
        result = self.store.wait_for_job(timeout=1)
        self.assertIsNone(result)

    def test_ack_job_removes_from_pending_list(self):
        """Messages ACKed after delivery should not be redelivered."""
        job = self.store.create("bulk-export", {})
        self.store.notify_new_job(job.id)
        result = self.store.wait_for_job(timeout=2)
        self.assertIsNotNone(result)
        _, message_id = result
        self.store.ack_job(message_id)
        # Attempt to claim this message as stale — it should not appear (it's been acked)
        stale = self.store.claim_stale_jobs(min_idle_ms=0)
        self.assertNotIn(job.id, stale)

    def test_claim_stale_jobs_returns_unacked_messages(self):
        """claim_stale_jobs reclaims messages delivered but not yet ACKed."""
        job = self.store.create("bulk-export", {})
        self.store.notify_new_job(job.id)
        # Receive but do NOT ack
        result = self.store.wait_for_job(timeout=2)
        self.assertIsNotNone(result)
        # Immediately try to claim stale with 0 ms threshold (everything pending)
        stale = self.store.claim_stale_jobs(min_idle_ms=0)
        self.assertIn(job.id, stale)
        # Now ack to clean up
        _, message_id = result
        self.store.ack_job(message_id)
    ...
    def test_get_queue_depth(self):
        ...
        self.assertEqual(self.store.get_queue_depth(), 0)
        j1 = self.store.create("bulk-export", {})
        j2 = self.store.create("cohort", {})
        self.store.notify_new_job(j1.id)
        self.store.notify_new_job(j2.id)
        depth = self.store.get_queue_depth()
        self.assertEqual(depth, 2)
        # Consume and ack both
        for _ in range(2):
            result = self.store.wait_for_job(timeout=2)
            if result:
                _, mid = result
                self.store.ack_job(mid)
