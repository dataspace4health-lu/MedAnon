...
class TestJobsRouter(unittest.TestCase):
    ...
    def test_submit_batch_patient_export_returns_202(self):
        resp = self.client.post(
            "/v1/jobs/batch-patient-export",
            json={"server_url": "http://fhir:8080/fhir", "patient_ids": ["P1", "P2"]},
        )
        self.assertEqual(resp.status_code, 202)
        body = resp.json()
        self.assertIn("job_id", body)
        self.assertEqual(body["status"], "pending")
        self.assertEqual(body["type"], "batch-patient-export")

    def test_batch_patient_export_stores_patient_ids(self):
        resp = self.client.post(
            "/v1/jobs/batch-patient-export",
            json={"server_url": "http://fhir:8080/fhir", "patient_ids": ["P1", "P2", "P3"]},
        )
        job_id = resp.json()["job_id"]
        job = self.store.get(job_id)
        self.assertEqual(job.params["patient_ids"], ["P1", "P2", "P3"])

    def test_batch_patient_export_empty_ids_422(self):
        resp = self.client.post(
            "/v1/jobs/batch-patient-export",
            json={"patient_ids": []},
        )
    ...
    def test_batch_patient_export_rejects_private_ip(self):
        resp = self.client.post(
            "/v1/jobs/batch-patient-export",
            json={
                "server_url": "http://169.254.169.254/latest/meta-data/",
