...
def _make_urllib3_response(data: bytes, status: int = 200):
...
class TestProxyAnalyseRisk(unittest.TestCase):
    def _call(self, body: bytes, content_type: str, mock_response: dict):
        """Call proxy_analyse_risk with a mocked proxy_request returning mock_response."""
        raw = json.dumps(mock_response).encode()
        resp = _make_urllib3_response(raw)

        with patch.dict(os.environ, {"ANALYTICS_SERVICE_URL": _ANALYTICS_URL}):
            with patch("integrations.http_client.proxy_request", return_value=resp):
    ...
    def test_http_error_raises_value_error(self):
        with patch.dict(os.environ, {"ANALYTICS_SERVICE_URL": _ANALYTICS_URL}):
            with patch(
                "integrations.http_client.proxy_request",
                side_effect=ValueError("Upstream service HTTP 500 for http://analytics:8100/v1/analyse/risk: server error"),
            ):
                from integrations.analytics.client import proxy_analyse_risk
    ...
    def test_url_error_raises_value_error(self):
        with patch.dict(os.environ, {"ANALYTICS_SERVICE_URL": _ANALYTICS_URL}):
            with patch(
                "integrations.http_client.proxy_request",
                side_effect=ValueError("Upstream service unreachable for http://analytics:8100/v1/analyse/risk: connection refused"),
            ):
                from integrations.analytics.client import proxy_analyse_risk
                with self.assertRaises(ValueError) as ctx:
                    proxy_analyse_risk(b"{}", "application/json")
    ...
    def test_uses_analytics_service_url_env(self):
        ...
        with patch.dict(os.environ, {"ANALYTICS_SERVICE_URL": "http://analytics:8100"}):
            with patch("integrations.http_client.proxy_request", side_effect=capture_request):
...
class TestProxyGenerateSynthetic(unittest.TestCase):
    def _call(self, body: bytes, content_type: str, params: dict, response_bytes: bytes):
        resp = _make_urllib3_response(response_bytes)
    ...
    def test_http_error_raises_value_error(self):
        with patch.dict(os.environ, {"ANALYTICS_SERVICE_URL": _ANALYTICS_URL}):
            with patch(
                "integrations.http_client.proxy_request",
                side_effect=ValueError("Upstream service HTTP 422"),
            ):
                from integrations.analytics.client import proxy_generate_synthetic
    ...
    def test_url_error_raises_value_error(self):
        with patch.dict(os.environ, {"ANALYTICS_SERVICE_URL": _ANALYTICS_URL}):
            with patch(
                "integrations.http_client.proxy_request",
                side_effect=ValueError("Upstream service unreachable"),
            ):
                from integrations.analytics.client import proxy_generate_synthetic
                with self.assertRaises(ValueError):
...
class TestRemoteNlpDetector(unittest.TestCase):
    def _mock_response(self, scrubbed_text: str, token_state: dict | None = None):
        ...
        return _make_urllib3_response(raw)
    ...
    def _call(self, text, mock_resp_or_exc, token_state=None):
        ...
        with patch.dict(os.environ, {"NLP_SERVICE_URL": _NLP_URL}):
            if isinstance(mock_resp_or_exc, Exception):
                with patch("integrations.http_client.proxy_request", side_effect=mock_resp_or_exc):
                    return analyze_and_replace_remote(
                        text, entities=["PERSON"], threshold=0.4,
                        language="en", mode="tokenize", token_state=ts,
                    ), ts
            else:
                with patch("integrations.http_client.proxy_request", return_value=mock_resp_or_exc):
                    return analyze_and_replace_remote(
                        text, entities=["PERSON"], threshold=0.4,
                        language="en", mode="tokenize", token_state=ts,
                    ), ts
    ...
    def test_http_error_returns_placeholder_text(self):
        original = "John Doe has hypertension"
        result, _ = self._call(original, ValueError("Upstream service HTTP 503"))
        self.assertEqual(result, "[NLP_UNAVAILABLE]")

    def test_url_error_returns_placeholder_text(self):
        original = "Jane Smith"
        result, _ = self._call(original, ValueError("Upstream service unreachable"))
        self.assertEqual(result, "[NLP_UNAVAILABLE]")

    def test_generic_exception_returns_placeholder_text(self):
